...
void savePathPlot (vector<Matrix> &poses_gt,vector<Matrix> &poses_result,string file_name) {
  ...
  for (int32_t i=0; i<poses_gt.size()-1; i+=step_size)
...
bool eval (string gt, string estimate,string result_dir,int i) {
...
//    string result_dir     = "results/" + result_sha;
    ...
    // read ground truth and result poses
    vector<Matrix> poses_gt     = loadPoses(gt_dir + "/" + file_name);
    vector<Matrix> poses_result = loadPoses(estimate + "/" + file_name);
...
int32_t main (int32_t argc,char *argv[]) {
    ...
    if (argc!=5) {
        cout << "Usage: ./gt eval_odometry result_dir sequence" << endl;
        return 1;
    }

    // read arguments
    string gt = argv[1];
    string eval_odometry = argv[2];
    string result_dir = argv[3];
    int idx = atoi(argv[4]);
    // run evaluation
    bool success = eval(gt,eval_odometry,result_dir,idx);
    return 0;
}
